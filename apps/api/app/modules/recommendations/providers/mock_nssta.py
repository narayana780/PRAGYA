...
class MockNSSTAProvider(LearningProvider):
    ...
    def provider_name(self) -> str:
        return "NSSTA / TPAC (Mock Catalogue)"
    ...
    async def get_learning_history(self, employee_id: str) -> list[ProviderCompletionStatus]:
        ...
        return [
            ProviderCompletionStatus(
                ...
                provider_item_id=r.programme_id or str(r.id),
    ...
    async def get_enrollment_status(
        ...
        stmt = select(TrainingHistory).where(
            TrainingHistory.employee_id == employee_id,
            (TrainingHistory.programme_id == provider_item_id) | (TrainingHistory.title.ilike(f"%{provider_item_id}%")),
        )
        res = await self.db.execute(stmt)
